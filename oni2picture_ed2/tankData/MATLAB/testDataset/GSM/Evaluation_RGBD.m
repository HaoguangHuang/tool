...
if nargin<1, Folder_Name = 'BootStraping'; end
...
res_mat = zeros(num_GT, 7+1);  % 8D detection accuracy + 1D MAE
maskBorder = imread('E:\dataSet\ICAISS\GSM\GSM_dataset\GSM\TimeOfDay_ds\TimeOfDay_ds.bmp');
maskBorder = maskBorder(:,:,1);
% maskBorder = ones(H,W);
for i=1:num_GT  % 'ColCamSeq'/ 'DCamSeq' / 'GenSeq' / 'ShSeq'
    str_gtname = GT_fname(idx(i)).name;
    frame_no = sscanf(str_gtname, '%d.bmp');
    ...
    Res_mask = logical(Res_mask) & logical(maskBorder);
    GT_mask  = logical(GT_mask)  & logical(maskBorder);
