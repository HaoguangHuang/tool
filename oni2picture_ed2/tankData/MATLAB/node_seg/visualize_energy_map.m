...
function visualize_energy_map(pc1, pc2, unique_corr, camera_para)
    ...
    delta_v = uvd_pc1(:,1) - uvd_pc2(:,1);  
    delta_u = uvd_pc1(:,2) - uvd_pc2(:,2);
    delta_d = uvd_pc1(:,3) - uvd_pc2(:,3);
    
    getEnergyMap_all_without_direction(delta_u, delta_v, delta_d, uvd_pc1, unique_corr, pixelRadius, depthRadius);
    getEnergyMap_with_direction(delta_u, uvd_pc1, unique_corr, pixelRadius, 'delta\_u');
    getEnergyMap_with_direction(delta_v, uvd_pc1, unique_corr, pixelRadius, 'delta\_v');
    getEnergyMap_with_direction(delta_d, uvd_pc1, unique_corr, depthRadius, 'delta\_d');
...
function uvd_pc = transformXYZ2UVD(xyz_pc, camera_para)
    ...
    for i = 1:size(xyz_pc,1)
        uvd_pc(i,2) = round(xyz_pc(i,1)*camera_para.fx/xyz_pc(i,3)+camera_para.cx);  %u = (x*f_x)/z + c_x
        uvd_pc(i,1) = round(xyz_pc(i,2)*camera_para.fy/xyz_pc(i,3)+camera_para.cy);  %v = (y*f_y)/z + c_y
    end
end
...
function getEnergyMap_all_without_direction(delta_u, delta_v, delta_d, uvd_pc1, unique_corr, pixelRadius, depthRadius)
    ...
    energyMap = zeros(480,640,3); count = 0;
    for i = 1:size(unique_corr,1)
        if uvd_pc1(i,1)<1 || uvd_pc1(i,1)>480 || uvd_pc1(i,2)< 1 || uvd_pc1(i,2)>640
            count = count + 1;
            continue;
        end
...
function getEnergyMap_with_direction(delta, uvd_pc1, unique_corr, Radius, choice)
    ...
    for i = 1:size(minus_R,1)
        u = uvd_pc1(minus_R(i),1); v = uvd_pc1(minus_R(i),2);
        energyMap(u, v, 1) = 255;
    end
    %%====construct G component via minus delta====
    for i = 1:size(zeros_G,1)
       u = uvd_pc1(zeros_G(i),1); v = uvd_pc1(zeros_G(i),2);
        energyMap(u, v, 2) = 255;
    end
    %%====construct B component via minus delta====
    for i = 1:size(positive_B,1)
        u = uvd_pc1(positive_B(i),1); v = uvd_pc1(positive_B(i),2);
        energyMap(u, v, 3) = 255;
    end
