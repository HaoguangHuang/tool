...
%                 It should be mention that each point can only match one node finally.
...
function [outlier_index, pc_bestNode_distr] = findBestNode(pc1, pc2, Tmat, pc_set1_node_index, layers, thres)
    if nargin < 4
        error('function FINDBESTNODE don''t have enough parameters!');
    elseif nargin < 5
        layers = 4; thres = 1;%mm
    elseif nargin < 6
        thres = 1;            %mm
    ...
    for i = tt(morethanone_index)'
        ...
        for n = find(pc_set1_node_index(i,:))
            ...
            if dist < mi
                pc_bestNode_distr(i,1) = n; mi = dist;
            end
        end
        %%========judege outlier=========
        dd(i,2) = mi;
    ...
    count = 1;t_thres = 0:0.1:3;
    for t = t_thres
    inlier_rate(1,count) = sum(dd(dd(:,2)<inf,2)<t,1)/pc1.Count;     % here point controlled by none node is regarded as outlier
    count = count + 1;
    end
    figure(100),bar(t_thres,inlier_rate,0.4);xlabel('thres/mm');ylabel('inlier rate');title('ICP result of layer 4');
    grid on;
